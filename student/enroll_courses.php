...
if(isset($_POST['enroll_student'])){
    ...
    if(mysqli_num_rows($check) == 0){
        ...
        if($course['price'] > 0){
            ...
            $subscription_type = 'monthly'; // aap ye front-end me select karwa sakte ho
            $amount = $course['price'];
            mysqli_query($conn,"INSERT INTO payments (user_id, course_id, amount, payment_method, status) 
                               VALUES ('$student_id','$course_id','$amount','credit_card','completed')");
            $msg = "<div class='alert alert-success'>Enrolled successfully. Payment of $$amount received.</div>";
        } else {
            $msg = "<div class='alert alert-success'>Enrolled successfully in free course.</div>";
        }
